...
이 문서는 `option-pricing` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 옵션 가격평가 (Option Pricing)
- 연구 카테고리: Derivatives Pricing & Volatility
- 핵심 질문: Monte Carlo 시뮬레이션은 옵션 가격을 얼마나 안정적으로 추정하며 해석해와 어떤 차이를 보이는가?
...
- KAIST-DFMBA/monte-carlo-pricing-simulation/

## 참고 초점

- GBM 경로 생성과 유럽형 옵션 Monte Carlo pricing
- Black-Scholes 해석해와 Monte Carlo 결과 비교
- Asian option, convergence, confidence interval 확장
